...
type propTypes = {
    ...
    productDetails: ProductDataModelWithColorMap,
}

const ProductsCard = ({ productDetails, itemClasses, parentClasses }: propTypes) => {
    ...
    return (
        <div id={`product_${productDetails._id}`} className={clsx("flex flex-col items-center cursor-pointer m-3 ", parentClasses ? parentClasses : '')}>
            <a href={`/products/${productDetails.productURL}`}>
                <div className={clsx("card card-compact glass min-h-[300px] md:min-h-[400px] 2xl:min-h-[500px] overflow-hidden rounded-none shadow-md p-0", itemClasses ? itemClasses : '', ProductsCardStyle.product_card)}>
                    <div className="w-40 h-40 md:w-72 md:h-72 2xl:w-80 2xl:h-80 overflow-hidden relative">
                        ...
                        {
                            parseFloat(productDetails.productDiscountPercentage) > 0 && <div className="mask mask-hexagon bg-red-500 text-white flex text-xs items-center absolute bottom-0 right-0 mr-2 mb-2 p-3">
                                {productDetails.productDiscountPercentage}
                            </div>
                        }
                    </div>
                    <div className="card-body items-center w-40 md:w-72 2xl:w-80">
