                        ...
                        Array.isArray(blogList) && blogList.length > 0 && blogList.map(blogItem => {
                            return (
                                <Link href={`/blog/${blogItem.slug}`} key={blogItem._id} >
                                    <article className="bg-white rounded-lg border border-gray-200 shadow-md card card-body">
                                        ...
                                        <div className='card-body'>
                                            <div className="flex justify-between items-center mb-5 text-gray-500">
                                                ...
                                                <span className="text-sm">{new Date(blogItem.updated).toLocaleDateString("en", {
                                                    dateStyle: "long"
                                                })}</span>
                                            </div>
                                            <h2 className="card-title cursor-pointer"><Link href={`/blog/${blogItem.slug}`}>{blogItem.title}</Link></h2>
                                            <p className="mb-5 font-light text-current">{blogItem.description}</p>
