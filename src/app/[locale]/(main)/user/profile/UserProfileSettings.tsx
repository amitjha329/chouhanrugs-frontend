...
const UserProfileSettings = ({ className, userData }: { className?: string, userData?: UserProfileDataModel }) => {
    ...
    const updateDetails = async (image?: string) => {
        try {
            ...
            if (res.ack) {
                await onPageNotifications("success", t('profileUpdated'))
                await refetch()
            } else {
                await onPageNotifications("error", t('profileUpdateError'))
            }
        } catch (err) {
            console.log(err)
            await onPageNotifications("error", t('profileUpdateError'))
    ...
    return (
        <div className={className}>
            ...
            <form onSubmit={profileUpdate} className="space-y-6">
                    ...
                    <div className="p-6">
                        <div className="flex flex-col sm:flex-row items-center gap-6">
                            ...
                            <div className="text-center sm:text-left">
                                <h3 className="font-medium text-base-content">{name || t('yourName')}</h3>
                ...
                <div className="bg-base-100 rounded-2xl border border-base-300/50 overflow-hidden">
                    <div className="px-6 py-4 border-b border-base-300/50 bg-gradient-to-r from-primary/5 to-transparent">
                        <h2 className="font-semibold text-base-content">{t('personalInfo')}</h2>
                        <p className="text-sm text-base-content/60 mt-0.5">{t('personalInfoSub')}</p>
                    </div>
                    <div className="p-6 space-y-5">
                        ...
                        <div className="form-control">
                            <label className="label">
                                ...
                                <span className="label-text-alt badge badge-ghost badge-sm">{t('cannotChange')}</span>
                            </label>
                            <input
                            ...
                            />
                            <label className="label">
                                <span className="label-text-alt text-base-content/50">{t('emailSupport')}</span>
                            </label>
                        </div>
                    </div>
                </div>
